...
@section('content-admin-card')
    ...
    <div class="card-body">
        {!! Form::open(['route' => ['hot-deals.update', $data->id], 'method' => 'PUT', 'enctype' => 'multipart/form-data', 'files' => true, 'id' => 'formData']) !!}
        <div class="row mt-3">
            {{ Form::label('title', 'Title', ['class' => 'col-sm-2 col-form-label']) }}
            <div class="col-sm-10">
                {{ Form::text('title', $data->title, ['class' => 'form-control','required' => 'required','placeholder' => 'Title']) }}
            </div>
        </div>
        <div class="row mt-3">
            {{ Form::label('subtitle', 'Subtitle', ['class' => 'col-sm-2 col-form-label']) }}
            <div class="col-sm-10">
                {{ Form::text('subtitle', $data->subtitle, ['class' => 'form-control','required' => 'required','placeholder' => 'Subtitle']) }}
            </div>
        </div>
        <div class="row mt-3">
            {{ Form::label('type', 'Type', ['class' => 'col-sm-2 col-form-label']) }}
            <div class="col-sm-10">
                {!! Form::select('type', ['product-category' => 'Product Categories', 'product-list' => 'Product List'], $data->type, ['class' => 'form-control', 'required' => 'required']) !!}
            </div>
        </div>
        ...
        <div id="category-list">
            <div class="row mt-3">
                ...
                <div class="col-md-4 col-sm-10">
                    {!! Form::select('category_id', $categories , isset($data->data->category_id) ? $data->data->category_id : 0, ['class' => 'form-control']) !!}
                </div>
            </div>
        </div>
        <div id="product-list">
            <div class="row mt-3">
                ...
                <div class="col-sm-10">
                    {!! Form::select('product_ids[]', $products, isset($data->data->product_ids) ? $data->data->product_ids : 0, ['class' => 'selectpicker form-control', 'multiple'=>'', 'data-live-search' => 'true']) !!}
                </div>
            </div>
        </div>
        <div id="product-total">
            <div class="row mt-3">
                ...
                <div class="col-md-4 col-sm-10">
                    {!! Form::number('product_total', isset($data->data->product_total) ? $data->data->product_total : null, ['class' => 'form-control', 'min' => 1, 'max' => 10]) !!}
                </div>
            </div>
        </div>
        <div id="button-link">
            <div class="row mt-3">
                ...
                <div class="col-sm-10">
                    {{ Form::text('button_link', isset($data->data->button_link) ? $data->data->button_link : null , ['class' => 'form-control']) }}
                </div>
            </div>
        </div>
        <div class="row mt-3">
            ...
            <div class="col-sm-10">
                {{ Form::text('button_text', $data->data->button_text, ['class' => 'form-control', 'required' => 'required', 'placeholder' => 'Button Text']) }}
            </div>
        </div>
        <div class="row mt-3">
